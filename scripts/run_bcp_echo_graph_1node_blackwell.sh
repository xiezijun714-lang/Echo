#!/bin/bash
# Multi-turn Search Tool-Calling ECHO-Graph on BrowseComp-Plus
# Single node × 8 Blackwell B cards, Qwen3-32B, Megatron backend, SGLang rollout
#
...
#
# Run from project root: bash scripts/run_bcp_echo_graph_1node_blackwell.sh
...
source "${PROJECT_DIR}/scripts/bcp/bcp_runtime.sh"
...
if [ -z "${TRAINER_IPS:-}" ] && [ -n "${PADDLE_TRAINERS:-}" ]; then
...
DATA_DIR="${DATA_DIR:-${PROJECT_DIR}/dataset/bcp}"
...
source "${PROJECT_DIR}/scripts/bcp/bcp_node_utils.sh"
select_bcp_nodes
...
BCP_CUDA_MAJOR="${BCP_CUDA_MAJOR:-13}"
...
ECHO_CREDIT_METHOD=${ECHO_CREDIT_METHOD:-graph}
...
AGENT_LOOP_WORKERS=${AGENT_LOOP_WORKERS:-64}
...
TOOL_CONFIG_TEMPLATE_PATH="${TOOL_CONFIG_TEMPLATE_PATH:-${PROJECT_DIR}/scripts/bcp/config/tool_config/search_tool_config.yaml}"
...
if bcp_preflight_complete verl.trainer.main_ppo "$PROJECT_DIR/scripts/bcp/config" bcp_multiturn_megatron_grpo; then
    exit 0
fi
...
RETRIEVER_CMD=("${VENV_PATH}/bin/python3"
    "$PROJECT_DIR/scripts/bcp/browsecomp_retrieval_server.py"
    --mode dense
...
"$PYTHON_BIN" -m verl.trainer.main_ppo \
    --config-path="$PROJECT_DIR/scripts/bcp/config" \
    ...
    +reward.custom_reward_function.path="$PROJECT_DIR/agent/bcp_llm_judge.py" \
