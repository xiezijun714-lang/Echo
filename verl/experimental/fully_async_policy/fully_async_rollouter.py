...
class FullyAsyncRollouter(SeparateRayPPOTrainer):
    ...
    def do_validate(self, validation_step=None, validation_dump_tag=None) -> ValidateMetrics:
        """Run validation and return metrics"""
        timing_raw = {}
        merged = validation_dump_tag is not None
        with marked_timer("rollouter/validate_time", timing_raw, color="green"):
            val_metrics: dict = self._validate(
                merged=merged,
                validation_step=validation_step,
                validation_dump_tag=validation_dump_tag,
            )
