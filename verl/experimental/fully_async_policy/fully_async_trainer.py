...
class FullyAsyncTrainer(SeparateRayPPOTrainer):
    ...
    async def _get_samples_from_queue(self) -> tuple[None, None] | tuple[int, Any]:
        ...
        balance_pad_to = self._get_dp_size(self.actor_rollout_wg, "actor") if self._is_supo else None
        if self.config.trainer.balance_batch:
            batch = assemble_batch_from_rollout_samples(
                queue_samples, self.tokenizer, self.config, self._balance_batch, balance_pad_to=balance_pad_to
            )
        else:
        ...
        if self.config.async_training.use_trainer_do_validate:
            ...
            try:
                with marked_timer("trainer/validate_time", self.timing_raw):
                    train_val_metrics = self._validate(
                        True,
                        validation_step=self.current_param_version,
                        validation_dump_tag="trainer",
                    )
            finally:
    ...
    async def _fit_validate(self, val_before_train=False):
        ...
        validation_dump_tag = "rollouter" if self.config.async_training.use_trainer_do_validate else None
        val_future = None
        try:
            val_future = self.rollouter.do_validate.remote(self.current_param_version, validation_dump_tag)
            ...
            if val_future is not None:
                ray.cancel(val_future, force=True)
            logger.exception("[FullyAsyncTrainer] validation failed; continue training")
            self.logger.log(
                data={
                    "val-aux/validation_failed": 1.0,
                    "timing_s/validation_failed_time": time.time() - val_start_time,
                },
                step=self.current_param_version,
            )
            print(f"[FullyAsyncTrainer] WARNING: validation failed and was skipped: {e}")
    ...
    def _save_checkpoint(self):
        ...
        actor_save_infos = self.actor_rollout_wg.save_checkpoint(
        ...
        self._sync_local_checkpoint_layout(
            local_global_step_folder=local_global_step_folder,
            actor_save_infos=actor_save_infos,
        )

    def _copy_file_between_nodes(self, src_ip: str, src_path: str, dst_ip: str, dst_path: str):
        local_ip = ray.util.get_node_ip_address().strip("[]")
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            if src_ip == local_ip:
                ...
                subprocess.run(["scp", "-q", f"{src_ip}:{src_path}", tmp_path], check=True)

            if dst_ip == local_ip:
                os.makedirs(os.path.dirname(dst_path), exist_ok=True)
                shutil.copy2(tmp_path, dst_path)
            else:
                subprocess.run(["ssh", dst_ip, "mkdir", "-p", os.path.dirname(dst_path)], check=True)
                subprocess.run(["scp", "-q", tmp_path, f"{dst_ip}:{dst_path}"], check=True)

    def _prepare_remote_dir(self, node_ip: str, path: str):
        subprocess.run(
            [
                "ssh",
                node_ip,
        ...
        subprocess.run(["ssh", node_ip, "mkdir", "-p", path], check=True)

    def _finalize_remote_dir(self, node_ip: str, tmp_path: str, dst_path: str):
        script = (
            f"rm -rf {shlex.quote(dst_path)}.old && "
            f"if [ -d {shlex.quote(dst_path)} ]; then mv {shlex.quote(dst_path)} {shlex.quote(dst_path)}.old; fi && "
            f"mv {shlex.quote(tmp_path)} {shlex.quote(dst_path)} && "
            f"rm -rf {shlex.quote(dst_path)}.old"
        )
        subprocess.run(["ssh", node_ip, script], check=True)

    def _copy_dir_between_nodes(self, src_ip: str, src_path: str, dst_ip: str, dst_path: str):
        if src_ip == dst_ip and src_path == dst_path:
            return
        local_ip = ray.util.get_node_ip_address().strip("[]")
        tmp_dst_path = f"{dst_path}.syncing"

        if dst_ip == local_ip:
            shutil.rmtree(tmp_dst_path, ignore_errors=True)
            os.makedirs(tmp_dst_path, exist_ok=True)
        else:
            self._prepare_remote_dir(dst_ip, tmp_dst_path)

        if src_ip == local_ip:
            if not os.path.isdir(src_path):
                raise FileNotFoundError(src_path)
            src_cmd = ["tar", "-C", src_path, "-cf", "-", "."]
        else:
            src_cmd = ["ssh", src_ip, "tar", "-C", src_path, "-cf", "-", "."]

        if dst_ip == local_ip:
            dst_cmd = ["tar", "-C", tmp_dst_path, "-xf", "-"]
        else:
            dst_cmd = ["ssh", dst_ip, "tar", "-C", tmp_dst_path, "-xf", "-"]
        ...
        dst_returncode = dst_proc.wait()
        src_returncode = src_proc.wait()
        if src_returncode != 0:
            raise subprocess.CalledProcessError(src_returncode, src_cmd)
        if dst_returncode != 0:
            raise subprocess.CalledProcessError(dst_returncode, dst_cmd)

        if dst_ip == local_ip:
            old_dst_path = f"{dst_path}.old"
            shutil.rmtree(old_dst_path, ignore_errors=True)
            if os.path.isdir(dst_path):
                os.rename(dst_path, old_dst_path)
            os.rename(tmp_dst_path, dst_path)
            shutil.rmtree(old_dst_path, ignore_errors=True)
        else:
            self._finalize_remote_dir(dst_ip, tmp_dst_path, dst_path)

    def _sync_local_checkpoint_layout(self, local_global_step_folder: str, actor_save_infos: list[dict[str, Any]]):
        if not self.config.trainer.get("sync_local_checkpoint_metadata", True):
            return
        if not actor_save_infos:
            print("[FullyAsyncTrainer] No actor save info returned; skip local checkpoint metadata sync")
            return

        rank0_info = next((info for info in actor_save_infos if int(info.get("rank", -1)) == 0), actor_save_infos[0])
        source_ip = rank0_info["node_ip"]
        source_actor_path = rank0_info["local_path"]

        node_to_actor_path = {}
        for info in actor_save_infos:
            node_to_actor_path.setdefault(info["node_ip"], info["local_path"])

        sidecar_relpaths = [
            os.path.join("dist_ckpt", ".metadata"),
            os.path.join("dist_ckpt", "common.pt"),
            os.path.join("dist_ckpt", "metadata.json"),
            "transformer_config.json",
        ]
        for node_ip, actor_path in sorted(node_to_actor_path.items()):
            for relpath in sidecar_relpaths:
                src_path = os.path.join(source_actor_path, relpath)
                dst_path = os.path.join(actor_path, relpath)
                if source_ip == node_ip and src_path == dst_path:
                    continue
                self._copy_file_between_nodes(source_ip, src_path, node_ip, dst_path)

        source_hf_path = os.path.join(source_actor_path, "huggingface")
        if self.config.trainer.get("sync_local_checkpoint_hf", True) and os.path.isdir(source_hf_path):
            for node_ip, actor_path in sorted(node_to_actor_path.items()):
                dst_hf_path = os.path.join(actor_path, "huggingface")
                if source_ip == node_ip and source_hf_path == dst_hf_path:
                    continue
                print(f"[FullyAsyncTrainer] Sync HuggingFace checkpoint from {source_ip} to {node_ip}: {dst_hf_path}")
                self._copy_dir_between_nodes(source_ip, source_hf_path, node_ip, dst_hf_path)
        ...
        if os.path.isdir(model_path):
            for filename in hf_sidecar_names:
                src_path = os.path.join(model_path, filename)
                if not os.path.exists(src_path):
                    continue
                for node_ip, actor_path in sorted(node_to_actor_path.items()):
                    dst_path = os.path.join(actor_path, filename)
                    self._copy_file_between_nodes(local_ip, src_path, node_ip, dst_path)
        ...
        dataloader_path = os.path.join(local_global_step_folder, "data.pt")
        for node_ip in sorted(node_to_actor_path):
            if node_ip == local_ip:
                continue
            self._copy_file_between_nodes(local_ip, tracker_path, node_ip, tracker_path)
            if os.path.exists(dataloader_path):
                remote_dataloader_path = os.path.join(
                    checkpoint_root, os.path.basename(local_global_step_folder), "data.pt"
                )
                self._copy_file_between_nodes(local_ip, dataloader_path, node_ip, remote_dataloader_path)

        print(
            "[FullyAsyncTrainer] Synced local checkpoint metadata for "
            f"step {self.current_param_version} from {source_ip} to {sorted(node_to_actor_path)}"
        )
