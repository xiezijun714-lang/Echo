...
def pad_supo_dummy_trajectories(data: "DataProto", mini_batch_size: int) -> tuple["DataProto", int]:
    ...
    if dummy_data.batch is not None:
        if "attention_mask" in dummy_data.batch.keys():
            attention_mask = dummy_data.batch["attention_mask"]
            prompt_len = dummy_data.batch["prompts"].shape[1] if "prompts" in dummy_data.batch.keys() else attention_mask.shape[1]
            compact_attention_mask = torch.zeros_like(attention_mask)
            for row_idx in range(attention_mask.shape[0]):
                prompt_mask = attention_mask[row_idx, :prompt_len].bool()
                valid_prompt_indices = torch.nonzero(prompt_mask, as_tuple=False).flatten()
                keep_idx = valid_prompt_indices[-1] if valid_prompt_indices.numel() > 0 else torch.tensor(
                    0, device=attention_mask.device
                )
                compact_attention_mask[row_idx, keep_idx] = 1
            dummy_data.batch["attention_mask"] = compact_attention_mask
