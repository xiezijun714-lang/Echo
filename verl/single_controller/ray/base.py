...
class RayResourcePool(ResourcePool):
    ...
    def get_placement_groups(self, strategy="STRICT_PACK", name=None, device_name="cuda"):
        ...
        if self.node_ips is not None:
            if len(self.node_ips) != len(pg_scheme):
                raise ValueError(
                    f"node_ips length {len(self.node_ips)} must match placement group count {len(pg_scheme)} "
                    f"for resource pool {self.name_prefix}"
                )
            for node_ip, bundles in zip(self.node_ips, pg_scheme):
                node_resource_name = f"node:{node_ip}"
                for pg_bundle in bundles:
                    pg_bundle[node_resource_name] = 0.001
        ...
        self.pgs = pgs if self.node_ips is not None else sort_placement_group_by_node_ip(pgs)
        return self.pgs
...
class ResourcePoolManager:
    ...
    resource_pool_node_ips: dict[str, list[str]] = field(default_factory=dict)
    ...
    def create_resource_pool(self):
        ...
        for resource_pool_name, process_on_nodes in self.resource_pool_spec.items():
            node_ips = self.resource_pool_node_ips.get(resource_pool_name)
...
class RayWorkerGroup(WorkerGroup):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self._ray_master_port_range is None:
            env_master_port_range = os.environ.get("VERL_MASTER_PORT_RANGE")
            if env_master_port_range:
                env_master_port_range = env_master_port_range.strip().strip("[]")
                port_start, port_end = env_master_port_range.split(",", 1)
                self._ray_master_port_range = [int(port_start), int(port_end)]
    ...
    def _init_with_resource_pool(
        ...
        ordered_pgs = pgs if getattr(resource_pool, "node_ips", None) is not None else sort_placement_group_by_node_ip(pgs)
        for pg_idx, pg in enumerate(ordered_pgs):
