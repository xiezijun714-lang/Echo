...
def is_echo_turn_policy_loss(name: str) -> bool:
    return str(name) in ECHO_TURN_POLICY_LOSS_MODES
...
def compute_supo_advantage(
    ...
    **kwargs,
    ...
    rollout-level GRPO advantage. Sparse ECHO credit modes route only its positive
    part, while dense credit keeps the signed advantage. Overlong rollouts receive
    zero advantage.
    """
    with torch.no_grad():
        ...
        if (
            credit_mode != "none"
            and not _ECHO_CREDIT_EMPTY_TARGET_WARNED
            and selected_traj_arr is None
            ...
            _ECHO_CREDIT_EMPTY_TARGET_WARNED = True
...
def _coarsen_advantages_to_echo_turns(
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    echo_turn_ids: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Promote token-level credit to whole ECHO turns for turn-level losses."""
    valid_mask = response_mask.bool()
    effective_advantages = torch.zeros_like(advantages)
    effective_mask = torch.zeros_like(valid_mask)
    nonzero_adv_mask = advantages.ne(0) & valid_mask

    for row_idx in range(advantages.shape[0]):
        row_valid_mask = valid_mask[row_idx]
        row_turn_ids = echo_turn_ids[row_idx]
        for turn_id in torch.unique(row_turn_ids[row_valid_mask]):
            turn_value = int(turn_id.item())
            if turn_value < 0:
                continue
            turn_mask = row_valid_mask & row_turn_ids.eq(turn_value)
            turn_nonzero_mask = turn_mask & nonzero_adv_mask[row_idx]
            if turn_nonzero_mask.any():
                turn_adv = advantages[row_idx][turn_nonzero_mask][0]
                effective_advantages[row_idx][turn_mask] = turn_adv
                effective_mask[row_idx][turn_mask] = True

        # Compression selection/summary policy tokens may carry credit without a
        # normal environment turn id. Treat credited aux tokens in this row as a
        # narrow pseudo-turn so token credit is not dropped.
        aux_mask = row_valid_mask & row_turn_ids.lt(0) & nonzero_adv_mask[row_idx]
        if aux_mask.any():
            aux_adv = advantages[row_idx][aux_mask][0]
            effective_advantages[row_idx][aux_mask] = aux_adv
            effective_mask[row_idx][aux_mask] = True

    return effective_advantages, effective_mask


@register_policy_loss("echo_turn_gspo")
@register_policy_loss("echo_turn")
@register_policy_loss("echo-turn")
def compute_policy_loss_echo_turn_gspo(
    old_log_prob: torch.Tensor,
    log_prob: torch.Tensor,
    advantages: torch.Tensor,
    response_mask: torch.Tensor,
    ...
    config: Optional[ActorConfig] = None,
    rollout_is_weights: torch.Tensor | None = None,
    echo_turn_ids: torch.Tensor | None = None,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """
    Compute ECHO turn-level GSPO loss.

    Importance ratios are geometric means over each ECHO turn. If token-level
    credit assignment marks any token in a turn, the whole turn is trained with
    the representative non-zero advantage for that turn.
    """

    assert config is not None
    assert isinstance(config, ActorConfig)
    if echo_turn_ids is None:
        raise ValueError("echo_turn_gspo requires echo_turn_ids in the actor batch.")

    clip_ratio_low = config.clip_ratio_low if config.clip_ratio_low is not None else config.clip_ratio
    clip_ratio_high = config.clip_ratio_high if config.clip_ratio_high is not None else config.clip_ratio

    echo_turn_ids = echo_turn_ids.to(device=response_mask.device)
    if echo_turn_ids.shape != response_mask.shape:
        raise ValueError(
            f"echo_turn_ids shape {tuple(echo_turn_ids.shape)} must match response_mask shape "
            f"{tuple(response_mask.shape)}"
        )

    effective_advantages, effective_mask = _coarsen_advantages_to_echo_turns(
        advantages=advantages,
        response_mask=response_mask,
        echo_turn_ids=echo_turn_ids,
    )
    ...
    valid_mask = response_mask.bool()
    for row_idx in range(log_prob.shape[0]):
        row_valid_mask = valid_mask[row_idx]
        row_turn_ids = echo_turn_ids[row_idx]
        for turn_id in torch.unique(row_turn_ids[row_valid_mask]):
            turn_value = int(turn_id.item())
            if turn_value < 0:
                continue
            turn_mask = row_valid_mask & row_turn_ids.eq(turn_value)
            log_rho_turn = torch.sum(negative_approx_kl[row_idx] * turn_mask) / turn_mask.sum().clamp(min=1)
            turn_log_ratio[row_idx][turn_mask] = log_rho_turn

        aux_mask = effective_mask[row_idx] & row_turn_ids.lt(0)
        if aux_mask.any():
            log_rho_aux = torch.sum(negative_approx_kl[row_idx] * aux_mask) / aux_mask.sum().clamp(min=1)
            turn_log_ratio[row_idx][aux_mask] = log_rho_aux

    log_turn_importance_ratio = log_prob - log_prob.detach() + turn_log_ratio.detach()
    log_turn_importance_ratio = torch.clamp(log_turn_importance_ratio, max=10.0)
    turn_importance_ratio = torch.exp(log_turn_importance_ratio)

    pg_losses1 = -effective_advantages * turn_importance_ratio
    pg_losses2 = -effective_advantages * torch.clamp(turn_importance_ratio, 1 - clip_ratio_low, 1 + clip_ratio_high)
    pg_losses = torch.maximum(pg_losses1, pg_losses2)

    if rollout_is_weights is not None:
        pg_losses = pg_losses * rollout_is_weights

    loss_mask = effective_mask & valid_mask
    pg_loss = agg_loss(
        loss_mat=pg_losses, loss_mask=loss_mask, loss_agg_mode="seq-mean-token-mean", **config.global_batch_info
    )
    ...
    turn_active = loss_mask.any(dim=-1).float().mean()
    active_tokens = loss_mask.float().sum()
    valid_tokens = valid_mask.float().sum().clamp(min=1.0)
    pg_metrics = {
        "actor/pg_clipfrac": pg_clipfrac.detach().item(),
        "actor/ppo_kl": ppo_kl.detach().item(),
        "actor/pg_clipfrac_lower": pg_clipfrac_lower.detach().item(),
        "actor/echo_turn/active_seq_frac": turn_active.detach().item(),
        "actor/echo_turn/active_token_frac": (active_tokens / valid_tokens).detach().item(),
    }
    return pg_loss, pg_metrics
