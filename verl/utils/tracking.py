    ...
    def __init__(self, project_name, experiment_name, default_backend: str | list[str] = "console", config=None):
        ...
        if "tracking" in default_backend or "wandb" in default_backend:
            ...
            if os.environ.get("VERL_WANDB_DEBUG", "").lower() in {"1", "true", "yes", "on"}:
                ...
                print("wandb id:", getattr(run, "id", None), flush=True)
                print("wandb url:", getattr(run, "url", None), flush=True)
                print("resumed:", getattr(run, "resumed", None), flush=True)
                print("offline:", getattr(run, "offline", None), flush=True)
                print("mode:", getattr(run_settings, "mode", None), flush=True)
